...
class Friday0DTEStrategy:
    ...
    def Execute(self):
        ...
        try:
            ...
            # Phase 4: Monitor positions after entry
            if current_time >= time(10, 35):
    ...
    def capture_market_open_prices(self):
        ...
        try:
            ...
            # Also capture VIX for regime analysis
            if hasattr(self.algo, 'vix_symbol') and self.algo.vix_symbol in self.algo.Securities:
                vix_open = float(self.algo.Securities[self.algo.vix_symbol].Price)
    ...
    def analyze_pre_market_move(self):
        ...
        try:
            ...
            if 'VIX' in self.market_open_prices:
                ...
                vix_current = float(self.algo.Securities[self.algo.vix_symbol].Price)
    ...
    def execute_tom_king_entry(self):
        ...
        try:
            ...
            max_bp = self.algo.params.get_vix_regime_bp_limit(
                float(self.algo.Securities[self.algo.vix_symbol].Price) if hasattr(self.algo, 'vix_symbol') else 18
            )
            ...
            for futures_symbol, analysis in self.pre_entry_analysis.items():
                ...
                # Check correlation limits
                if hasattr(self.algo, 'correlation_manager'):
                    allowed, message = self.algo.correlation_manager.can_add_position(
                        futures_symbol, 'EQUITY_INDEX'
                    )
                    if not allowed:
                        self.algo.Log(f"⚠️ {futures_symbol}: {message}")
                        continue
    ...
    def calculate_position_size(self, futures_symbol, vix_adjustment=1.0):
        """Calculate position size based on account, BP, and VIX"""
        account_value = self.algo.Portfolio.TotalPortfolioValue
        
        # Base size from account phase
        if account_value < 40000:
    ...
    def execute_put_condor_with_real_chains(self, futures_symbol, vix_adjustment=1.0):
        ...
        try:
            ...
            for kvp in option_chains:
                ...
                # Find ATM and select strikes
                atm_idx = min(range(len(puts)), key=lambda i: abs(puts[i].Strike - underlying_price))
                
                # Put condor: Buy OTM, Sell closer to ATM
                if atm_idx >= 3:
                    long_put_far = puts[atm_idx - 3]    # Furthest OTM
                    short_put_1 = puts[atm_idx - 2]     # Short strike 1
                    short_put_2 = puts[atm_idx - 1]     # Short strike 2
                    long_put_near = puts[atm_idx]       # Closest to ATM
                    ...
                    credit = (short_put_1.BidPrice + short_put_2.BidPrice - 
    ...
    def execute_call_condor_with_real_chains(self, futures_symbol, vix_adjustment=1.0):
        ...
        try:
            ...
            for kvp in option_chains:
                ...
                # Find ATM and select strikes
                atm_idx = min(range(len(calls)), key=lambda i: abs(calls[i].Strike - underlying_price))
                
                # Call condor: Buy OTM, Sell closer to ATM
                if atm_idx < len(calls) - 3:
                    long_call_near = calls[atm_idx]       # Closest to ATM
                    short_call_1 = calls[atm_idx + 1]     # Short strike 1
                    short_call_2 = calls[atm_idx + 2]     # Short strike 2  
                    long_call_far = calls[atm_idx + 3]    # Furthest OTM
                    ...
                    credit = (short_call_1.BidPrice + short_call_2.BidPrice - 
    ...
    def can_enter_position(self, account_phase, active_positions, correlation_manager):
        ...
        # Check correlation limits
        if hasattr(correlation_manager, 'get_group_exposure'):
            equity_exposure = correlation_manager.get_group_exposure('A1')  # Equity index group
            if equity_exposure >= 3:  # Max 3 in equity index group
                return False, "Correlation group A1 limit reached"
