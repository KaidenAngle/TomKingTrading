...
class LongTerm112Strategy:
    ...
    def calculate_position_size(self, account_value, account_phase, vix_level=None):
        """
        Calculate LT112 position size based on account value and phase
        Returns number of spreads to trade
        """
        # Base position sizing by phase (PDF Page 16)
        base_sizing = {
            2: min(4, max(1, int(account_value / 12500))),  # Phase 2: MES, ~£12.5k per position
            3: 1,  # Phase 3: ES, single position focus
            4: min(3, max(1, int(account_value / 25000)))   # Phase 4: ES, ~£25k per position
        }
        
        position_count = base_sizing.get(account_phase, 1)
        
        # VIX adjustments (PDF Page 33)
        if vix_level:
            if vix_level < 15:
                position_count = max(1, int(position_count * 0.8))  # Reduce in low VIX
            elif vix_level > 25:
                position_count = max(1, int(position_count * 0.6))  # Reduce in high VIX
            elif vix_level > 30:
                position_count = min(position_count, 2)  # Maximum 2 in extreme VIX
