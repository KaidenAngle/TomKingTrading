...
class PhaseManager:
    def __init__(self, algorithm):
        ...
        # Define account phases per Tom King methodology (USD - converted from GBP at 1.27 rate)
        self.phases = {
            1: {
                'min_balance': 38100,  # $38,100 (£30k * 1.27)
                'max_balance': 50800,  # $50,800 (£40k * 1.27)
            ...
            2: {
                'min_balance': 50800,  # $50,800 (£40k * 1.27)
                'max_balance': 76200,  # $76,200 (£60k * 1.27)
                'strategies': ['FRIDAY_0DTE', 'LONG_TERM_112', 'FUTURES_STRANGLES', 
            ...
            3: {
                'min_balance': 76200,  # $76,200 (£60k * 1.27)
                'max_balance': 95250,  # $95,250 (£75k * 1.27)
                'strategies': ['FRIDAY_0DTE', 'LONG_TERM_112', 'FUTURES_STRANGLES',
                             'IPMCC', 'CALENDARIZED_112', 'ENHANCED_STRANGLES', 
            ...
            4: {
                'min_balance': 95250,  # $95,250 (£75k * 1.27)
    ...
    def IsStrategyAllowed(self, strategy_name: str) -> bool:
        ...
        # Define all valid Tom King strategies
        valid_strategies = [
            'FRIDAY_0DTE', 'LONG_TERM_112', 'FUTURES_STRANGLES',
            'IPMCC', 'CALENDARIZED_112', 'ENHANCED_STRANGLES',
            'BEAR_TRAP_11X', 'ADVANCED_0DTE', 'LEAP_PUT_LADDERS',
            'SECTION_9B_ADVANCED', 'POOR_MANS_COVERED_CALL',
            'ENHANCED_BUTTERFLY', 'RATIO_SPREADS'
        ]
        
        # First check if strategy is valid
    ...
    def ApplyWisdomRules(self, trade_params: dict) -> dict:
        ...
        if trade_params.get('strategy') == 'FRIDAY_0DTE':
            if self.algo.Time.hour < 10 or (self.algo.Time.hour == 10 and self.algo.Time.minute < 30):
                ...
                # Ensure allowed is set to True if not already specified
                if 'allowed' not in trade_params:
                    trade_params['allowed'] = True
        
        return trade_params
    
    def DeterminePhase(self, portfolio_value: float) -> dict:
        """Determine account phase and return detailed information"""
        for phase_num, phase_data in self.phases.items():
            if phase_data['min_balance'] <= portfolio_value < phase_data['max_balance']:
                return {
                    'phase': phase_num,
                    ...
                    'min_balance': phase_data['min_balance'],
                    'max_balance': phase_data['max_balance'],
                    'strategies': phase_data['strategies'],
                    'max_positions': phase_data['max_positions'],
                    'max_risk_per_trade': phase_data['max_risk_per_trade']
                }
        ...
        return {
            'phase': 1,
            'description': self.phases[1]['description'],
            'min_balance': self.phases[1]['min_balance'],
            'max_balance': self.phases[1]['max_balance'],
            'strategies': self.phases[1]['strategies'],
            'max_positions': self.phases[1]['max_positions'],
            'max_risk_per_trade': self.phases[1]['max_risk_per_trade']
        }
    
    def GetAvailableStrategies(self, phase: int) -> list:
        """Get list of available strategies for given phase"""
        phase_data = self.phases.get(phase, self.phases[1])
        
        if phase_data['strategies'] == 'ALL':
            # Return all known strategies for Phase 4
            return ['FRIDAY_0DTE', 'LONG_TERM_112', 'FUTURES_STRANGLES',
                   'IPMCC', 'POOR_MANS_COVERED_CALL', 'LEAP_PUT_LADDERS',
                   'ENHANCED_BUTTERFLY', 'RATIO_SPREADS', 'BEAR_TRAP_11X',
                   'ADVANCED_0DTE', 'CALENDARIZED_112', 'ENHANCED_STRANGLES',
                   'SECTION_9B_ADVANCED']
        
        return phase_data['strategies']
