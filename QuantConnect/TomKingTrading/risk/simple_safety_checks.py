...
class SimpleSafetyChecks:
    ...
    def __init__(self, algorithm):
        ...
        self.algo.Log("✅ Simple Safety Checks Active")
    ...
    def check_before_trade(self) -> bool:
        ...
        if daily_loss > self.max_daily_loss:
            self.algo.Log(f"❌ Daily loss limit hit: {daily_loss:.1%}")
            self.can_trade = False
            return False
        
        # Check 2: Too many losses today
        if self.losses_today >= 3:
            self.algo.Log(f"❌ Too many losses today: {self.losses_today}")
            self.can_trade = False
            return False
    ...
    def check_position_size(self, proposed_risk: float) -> bool:
        ...
        if risk_percent > self.max_loss_per_trade:
            self.algo.Log(f"❌ Position too large: {risk_percent:.1%} of account")
            return False
...
class SimpleAlerts:
    ...
    def send_alert(self, level: str, message: str):
        ...
        if level == "CRITICAL":
            self.algo.Error(f"🚨 CRITICAL: {message}")
            # In production: send SMS/email
            
        elif level == "WARNING":
            self.algo.Log(f"⚠️ WARNING: {message}")
            # In production: send email
            
        elif level == "INFO":
            self.algo.Log(f"ℹ️ INFO: {message}")
