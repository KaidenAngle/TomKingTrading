...
class PositionSyncBridge:
    ...
    def sync_new_position_to_active_list(self, position_id: str, strategy: str, symbol: str, 
        ...
        try:
            ...
            self.algo.Log(f"🔄 Synced PSM position {position_id} to active_positions")
        ...
        try:
            ...
            for active_pos in self.algo.active_positions:
                if active_pos.get('id') == position_id:
                    ...
                    self.algo.Log(f"🔄 Marked position {position_id} as closed in active_positions")
                    return True
        ...
        try:
            ...
            for active_pos in self.algo.active_positions:
                if active_pos.get('id') == position_id:
                    ...
                    self.algo.Log(f"🔄 Synced component {component_id} close to active_positions")
                    return True
            ...
            for active_pos in self.algo.active_positions:
                ...
                if active_pos.get('psm_managed'):
                    ...
                    if psm_position:
                        ...
                        self.algo.Log(f"⚠️ PSM position {position_id} not found for sync")
    ...
    def clean_closed_positions(self) -> int:
        ...
        try:
            ...
            if cleaned_count > 0:
                self.algo.Log(f"🧹 Cleaned {cleaned_count} closed positions from tracking")
    ...
    def validate_sync_integrity(self) -> Dict:
        ...
        try:
            ...
            if not integrity_report['is_synchronized']:
                self.algo.Log(f"⚠️ SYNC WARNING: PSM has {psm_positions} positions, "
