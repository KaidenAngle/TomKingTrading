...
class SystemValidator:
    ...
    def _validate_core_components(self):
        ...
        for attr, name in required_components:
            if hasattr(self.algo, attr):
                self.info.append(f"✅ {name} initialized")
            else:
                self.critical_issues.append(f"❌ {name} NOT initialized")
    ...
    def _validate_strategies(self):
        ...
        if hasattr(self.algo, 'friday_0dte'):
            try:
                ...
                self.info.append(f"✅ Friday 0DTE validation: {reason}")
            except Exception as e:
                self.critical_issues.append(f"❌ Friday 0DTE error: {e}")
        ...
        if hasattr(self.algo, 'lt112_strategy'):
            try:
                ...
                self.info.append(f"✅ LT112 validation: {reason}")
            except Exception as e:
                self.critical_issues.append(f"❌ LT112 error: {e}")
    ...
    def _validate_risk_management(self):
        ...
        if hasattr(self.algo, 'correlation_manager'):
            try:
                ...
                self.info.append(f"✅ Correlation limits working")
            except Exception as e:
                self.critical_issues.append(f"❌ Correlation manager error: {e}")
        ...
        if hasattr(self.algo, 'vix_manager'):
            try:
                ...
                if regime:
                    self.info.append(f"✅ VIX regime detected: {regime.get('name', 'Unknown')}")
                else:
                    self.warnings.append("⚠️ VIX regime not available")
            except Exception as e:
                self.critical_issues.append(f"❌ VIX manager error: {e}")
    ...
    def _validate_data_access(self):
        ...
        if self.algo.Securities:
            ...
            self.info.append(f"✅ Securities: {equity_count} equities, "
                           f"{futures_count} futures, {option_count} options")
            
            if equity_count == 0:
                self.warnings.append("⚠️ No equity securities subscribed")
        else:
            self.critical_issues.append("❌ No securities subscribed")
        ...
        if hasattr(self.algo, 'position_size_calculator'):
            try:
                ...
                for strategy, account, phase, vix in test_cases:
                    ...
                    if result['allowed']:
                        self.info.append(f"✅ {strategy} sizing: {result['contracts']} contracts")
                    else:
                        self.info.append(f"ℹ️ {strategy} not allowed at phase {phase}")
                        
            except Exception as e:
                self.warnings.append(f"⚠️ Position calculator not available: {e}")
        else:
            self.warnings.append("⚠️ Position size calculator not initialized")
    ...
    def _validate_correlation_limits(self):
        ...
        try:
            ...
            for account in test_accounts:
                ...
                self.info.append(f"✅ ${account:,} limits: {limits['max_total_positions']} positions")
                
        except Exception as e:
            self.critical_issues.append(f"❌ Correlation config error: {e}")
    ...
    def _validate_vix_integration(self):
        ...
        if hasattr(self.algo, 'vix_manager'):
            ...
            if self.algo.vix_manager.current_vix:
                self.info.append(f"✅ VIX data available: {self.algo.vix_manager.current_vix:.1f}")
            else:
                self.warnings.append("⚠️ VIX data not yet available")
                
            # Check VIX regime adjustments
            try:
                for vix in [10, 18, 25, 35]:
                    regime = self.algo.vix_manager.get_regime_for_vix(vix)
                    self.info.append(f"✅ VIX {vix}: {regime['name']}")
            except:
                self.warnings.append("⚠️ VIX regime lookup issues")
    ...
    def _validate_option_chains(self):
        ...
        if hasattr(self.algo, 'option_chain_manager'):
            try:
                ...
                if subs:
                    self.info.append(f"✅ Option subscriptions: {list(subs.keys())}")
                else:
                    self.warnings.append("⚠️ No option subscriptions active")
                ...
                self.critical_issues.append(f"❌ Option chain manager error: {e}")
        else:
            self.critical_issues.append("❌ Option chain manager not initialized")
    ...
    def _validate_order_execution(self):
        ...
        if hasattr(self.algo, 'option_order_executor'):
            self.info.append("✅ Option order executor available")
        else:
            self.warnings.append("⚠️ Option order executor not initialized")
            
        if hasattr(self.algo, 'strategy_order_executor'):
            self.info.append("✅ Strategy order executor available")
        else:
            self.warnings.append("⚠️ Strategy order executor not initialized")
    ...
    def _validate_phase_progression(self):
        ...
        if hasattr(self.algo, 'account_phase'):
            ...
            if current_phase == expected_phase:
                self.info.append(f"✅ Phase {current_phase} correct for ${account_value:,.0f}")
            else:
                self.warnings.append(f"⚠️ Phase mismatch: Current {current_phase}, "
            ...
            self.info.append(f"✅ Phase {current_phase} strategies: {', '.join(available)}")
        else:
            self.critical_issues.append("❌ Account phase not initialized")
    ...
    def _generate_report(self):
        ...
        if len(self.critical_issues) == 0:
            if len(self.warnings) == 0:
                ...
                report.append(f"\n⚠️ SYSTEM STATUS: OPERATIONAL WITH {len(self.warnings)} WARNINGS")
        else:
            report.append(f"\n❌ SYSTEM STATUS: {len(self.critical_issues)} CRITICAL ISSUES FOUND")
        ...
        if readiness_score >= 95:
            ...
            report.append("⚠️ System needs minor fixes before production")
        else:
            report.append("❌ System requires significant work before production")
    ...
    def quick_check(self):
        ...
        if not hasattr(self.algo, 'friday_0dte'):
            critical_ok = False
            checks.append("❌ Core strategies missing")
        
        if not hasattr(self.algo, 'vix_manager'):
            critical_ok = False
            checks.append("❌ VIX manager missing")
            
        if not hasattr(self.algo, 'correlation_manager'):
            critical_ok = False
            checks.append("❌ Correlation manager missing")
            
        if not self.algo.Securities:
            critical_ok = False
            checks.append("❌ No securities subscribed")
        
        if critical_ok:
            return "✅ Quick check passed - all critical components present"
        else:
            return f"❌ Quick check failed:\n" + "\n".join(checks)