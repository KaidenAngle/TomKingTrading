...
class OrderValidationSystem:
    ...
    def __init__(self, algorithm):
        ...
        self.algorithm.Log("✅ ORDER VALIDATION SYSTEM INITIALIZED")
    ...
    def validate_multi_leg_fills(self, group_id: str, timeout_seconds: int = None) -> bool:
        ...
        if all_filled:
            self.algorithm.Log(f"✅ Multi-leg order {group_id} fully filled")
            return True
        else:
    ...
    def validate_fill_price(self, order_id: int, order) -> bool:
        ...
        if expected > 0:
            ...
            if slippage > self.max_slippage_percent:
                self.algorithm.Log(f"⚠️ High slippage on {order_info['symbol']}: {slippage:.2%}")
    ...
    def handle_failed_multi_leg(self, group_id: str, reason: str):
        ...
        self.algorithm.Error(f"🚨 Multi-leg order {group_id} failed: {reason}")
    ...
    def handle_partial_multi_leg(self, group_id: str, partial_order_ids: List[int]):
        ...
        self.algorithm.Error(f"⚠️ Partial fills on multi-leg {group_id}")
