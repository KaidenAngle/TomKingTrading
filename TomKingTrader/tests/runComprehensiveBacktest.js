...
// Deterministic test data generator for reproducible test results
...
async function runSingleBacktest(engine, strategies, riskManager, config) {
    ...
    const numTrades = Math.floor(testGen.between(20, 70));
    const baseWinRate = config.config === 'Tom King Recommended' ? 0.75 : 0.65;
    
    for (let i = 0; i < numTrades; i++) {
        const isWin = testGen.next() < baseWinRate;
        trades.push({
            id: `BT-${i}`,
            symbol: testGen.choice(['SPY', 'QQQ', 'IWM']),
            strategy: testGen.choice(config.strategies),
    ...
    let cumulative = 0;
    for (let i = 0; i < 30; i++) {
        const dayPnL = (testGen.next() - 0.45) * 500; // Slight positive bias
        cumulative += dayPnL;
        dailyPnL.push({
